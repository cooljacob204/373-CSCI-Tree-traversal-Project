...
public:
	...
	bool findAndDeleteByCopying(const T&);
	...
	int nNodes() {//Node counter
		return nNodes(root);
	}
	int lNodes() {//Leaf counter.
		return lNodes(root);
	}
	void tMirror() {//Mirror
		return tMirror(root);
	}
	int maxHeight() {//Adding height
		return maxHeight(root);
	}
	void display() {
		display(root);
	}
	int pNode(int x) {//Assuming perfectly balanced, full tree.
		if (x < 1)//used for my version of display for personal bugfixing
			return 1;
		else
			return (pow(2,x) + pNode(x-1));
	}

protected:
	void display(BSTNode<T>*);
	int maxHeight(BSTNode<T>*);
	int nNodes(BSTNode<T>*);//Node counter
	int lNodes(BSTNode<T>*);//leaf counter
	void tMirror(BSTNode<T>*);//Mirror
	BSTNode<T>* root;
...
template<class T>
void BST<T>::display(BSTNode<T>* a) {//Wanted a better visual of the memory so I can confirm my added functions were working. Not part of assignment.
	if (maxHeight() > 8) {
		cout << "Tree to big, only usable with height of 7 or less";
		return;
	}
	int x = pNode(maxHeight()-1);
	BSTNode<T> ** temp = new BSTNode<T>*[x];//Loading it into an array, using the new equations we learned.
	for (int i = 0; i < x; i++)
		temp[i] = NULL;//For some reason it was pointing the array items to 0xCDCDCDCD
	temp[0] = a;
	for (int i = 0; i < x; i++) {
		if (temp[i]) {
			if (temp[i]->left != NULL)
	...
	int j = pow(2, maxHeight()-1) - 1, b = 0, level = 1;
	bool nLevel = true;
	for (int i = 0; i < x; i++) {
		if (i >= pow(2, level)-1 && i!=0) {
			level++;
			nLevel = true;
			cout << endl;
			b = j - 1;
...
template<class T>
int BST<T>::maxHeight(BSTNode<T> *p) {//Max Height.
	if (!p) return 0;
...
template<class T>
int BST<T>::nNodes(BSTNode<T> * p) {//Node Counter
	return (p->left ? nNodes(p->left) : 0) + (p->right ? nNodes(p->right) : 0) + 1;
}

template<class T>
int BST<T>::lNodes(BSTNode<T> * p) {//leaf Counter
	if (p->left == NULL && p->right == NULL)
		return 1;
	return (p->left ? lNodes(p->left) : 0) + (p->right ? lNodes(p->right) : 0);
}

template<class T>
void BST<T>::tMirror(BSTNode<T> * p) {//Mirror
	if (p == NULL)
		return;
...
template<class T>
bool BST<T>::findAndDeleteByCopying(const T& el) {
	BSTNode<T> *p = root, *prev = 0;
	while (p != 0 && !(p->el == el)) {
		prev = p;
		if (el < p->el)
